...
***Bite.*** *Melee Weapon Attack:* `+5` to hit, reach 5 ft., one target. *Hit:* 8 (`2d4 + 3`) piercing damage, and the target must succeed on a DC 12 Constitution saving throw or be [stunned](z_CLI/rules/conditions.md#Stunned) until the end of its next turn. On a failed save, the target begins to hallucinate and is afflicted with a [short-term madness](z_CLI/tables/short-term-madness.md) effect (determined randomly or by the DM; see "Madness" in chapter 8 of the "Dungeon Master's Guide"). The effect lasts 10 minutes.
```
^statblock