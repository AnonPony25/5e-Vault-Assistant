...
***Split.*** Trigger: While the jelly is Large or Medium and has 10+ [Hit Points](z_CLI/rules/variant-rules/hit-points-xphb.md), it becomes [Bloodied](z_CLI/rules/conditions.md#Bloodied) or is subjected to Lightning or Slashing damage. _Response:_ The jelly splits into two new Ochre Jellies. Each new jelly is one size smaller than the original jelly and acts on its [Initiative](z_CLI/rules/variant-rules/initiative-xphb.md). The original jelly's [Hit Points](z_CLI/rules/variant-rules/hit-points-xphb.md) are divided evenly between the new jellies (round down).
```
^statblock
