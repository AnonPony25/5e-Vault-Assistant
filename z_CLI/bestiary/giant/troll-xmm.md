...
***Loathsome Limbs (4/Day).*** If the troll ends any turn [Bloodied](z_CLI/rules/conditions.md#Bloodied) and took 15+ Slashing damage during that turn, one of the troll's limbs is severed, falls into the troll's space, and becomes a [Troll Limb](z_CLI/bestiary/giant/troll-limb-xmm.md). The limb acts immediately after the troll's turn. The troll has 1 [Exhaustion](z_CLI/rules/conditions.md#Exhaustion) level for each missing limb, and it grows replacement limbs the next time it regains [Hit Points](z_CLI/rules/variant-rules/hit-points-xphb.md).

***Regeneration.*** The troll regains 15 [Hit Points](z_CLI/rules/variant-rules/hit-points-xphb.md) at the start of each of its turns. If the troll takes Acid or Fire damage, this trait doesn't function on the troll's next turn. The troll dies only if it starts its turn with 0 [Hit Points](z_CLI/rules/variant-rules/hit-points-xphb.md) and doesn't regenerate.
...
***Charge.*** The troll moves up to half its [Speed](z_CLI/rules/variant-rules/speed-xphb.md) straight toward an enemy it can see.
```
^statblock
