...
***Chilling Gaze.*** *Constitution Saving Throw:* DC 13, one creature the yeti can see within 30 feet. *Failure:* 5 (`2d4`) Cold damage, and the target has the [Paralyzed](z_CLI/rules/conditions.md#Paralyzed) condition until the start of the yeti's next turn unless the target has [Immunity](z_CLI/rules/variant-rules/immunity-xphb.md) to Cold damage. *Success:* The target is immune to the Chilling Gaze of all yetis (but not abominable yetis) for 1 hour.
```
^statblock
