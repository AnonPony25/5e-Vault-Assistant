...
***Piercing Claw.*** *Melee Weapon Attack:* `+8` to hit, reach 15 ft., one target. *Hit:* 20 (`3d10 + 4`) piercing damage plus 11 (`2d10`) necrotic damage. If the target is a creature, the boneclaw can pull the target up to 10 feet toward itself, and the target is [grappled](z_CLI/rules/conditions.md#Grappled) (escape DC 14). The boneclaw has two claws. While a claw grapples a target, the claw can attack only that target.
...
***Shadow Stealth.*** While in dim light or darkness, the boneclaw takes the [Hide](z_CLI/rules/actions.md#Hide) action.
