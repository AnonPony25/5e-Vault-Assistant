...
***Illumination.*** The wisp sheds [Bright Light](z_CLI/rules/variant-rules/bright-light-xphb.md) in a 20-foot radius and [Dim Light](z_CLI/rules/variant-rules/dim-light-xphb.md) for an additional 20 feet.

***Incorporeal Movement.*** The wisp can move through other creatures and objects as if they were [Difficult Terrain](z_CLI/rules/variant-rules/difficult-terrain-xphb.md). It takes 5 (`1d10`) Force damage if it ends its turn inside an object.
...
***Consume Life.*** *Constitution Saving Throw:* DC 10, one living creature the wisp can see within 5 feet that has 0 [Hit Points](z_CLI/rules/variant-rules/hit-points-xphb.md). *Failure:* The target dies, and the wisp regains 10 (`3d6`) [Hit Points](z_CLI/rules/variant-rules/hit-points-xphb.md).

***Vanish.*** The wisp and its light have the [Invisible](z_CLI/rules/conditions.md#Invisible) condition until the wisp's [Concentration](z_CLI/rules/conditions.md#Concentration) ends on this effect, which ends early immediately after the wisp makes an attack roll or uses Consume Life.
```
^statblock
