...
```ad-statblock
title: Arveiaturace
![](https://raw.githubusercontent.com/5etools-mirror-3/5etools-img/main/bestiary/tokens/IDRotF/Arveiaturace.webp#token)
...
- **Senses** [blindsight](z_CLI/rules/senses.md#Blindsight) 60 ft. (blind beyond this radius), [darkvision](z_CLI/rules/senses.md#Darkvision) 60 ft., passive Perception 23
...
Legendary Action Uses: 3. Immediately after another creature's turn, Arveiaturace can expend a use to take one of the following actions. Arveiaturace regains all expended uses at the start of each of their turns.

***Detect.*** Arveiaturace makes a Wisdom ([Perception](z_CLI/rules/skills.md#Perception)) check.

***Tail Attack.*** Arveiaturace makes a tail attack.

***Wing Attack (Costs 2 Actions).*** Arveiaturace beats its wings. Each creature within 15 feet of Arveiaturace must succeed on a DC 22 Dexterity saving throw or take 15 (`2d6 + 8`) bludgeoning damage and be knocked [prone](z_CLI/rules/conditions.md#Prone). Arveiaturace can then fly up to half its flying speed.
```
^statblock