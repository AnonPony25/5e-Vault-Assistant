---
obsidianUIMode: preview
cssclasses:
- json5e-monster
tags:
- ttrpg-cli/compendium/src/5e/wdh
- ttrpg-cli/monster/cr/2
- ttrpg-cli/monster/size/medium
- ttrpg-cli/monster/type/humanoid/tiefling
aliases:
- "Tissina Khyret"
---
# Tissina Khyret
*Source: Waterdeep: Dragon Heist p. 116*  

Madame Khyret is the personal attendant to Ammalia Cassalanter and is the family's chief housemaid. Madame Khyret dresses in a long black gown and wears a headdress with black tassels. She is an aging tiefling and she spends most of her time in the master sitting room

```ad-statblock
title: Tissina Khyret
![](https://raw.githubusercontent.com/5etools-mirror-3/5etools-img/main/bestiary/tokens/WDH/Tissina%20Khyret.webp#token)
*Medium humanoid (tiefling), Lawful Evil*

- **Armor Class** 13 ([leather armor](z_CLI/items/leather-armor-xphb.md))
- **Hit Points** 33 (`6d8 + 6`)
- **Speed** 30 ft.

|STR|DEX|CON|INT|WIS|CHA|
|:---:|:---:|:---:|:---:|:---:|:---:|
|11 (+0)|14 (+2)|12 (+1)|10 (+0)|13 (+1)|14 (+2)|

- **Proficiency Bonus** +2
- **Saving Throws** ⏤
- **Skills** [Deception](z_CLI/rules/skills.md#Deception) +4, [Persuasion](z_CLI/rules/skills.md#Persuasion) +4, [Religion](z_CLI/rules/skills.md#Religion) +2
- **Senses** [darkvision](z_CLI/rules/senses.md#Darkvision) 60 ft., passive Perception 11
- **Damage Resistances** fire
- **Gear** [dagger](z_CLI/items/dagger-xphb.md)
- **Languages** Common, Infernal
- **Challenge** 2

## Traits

***Spellcasting.*** Tissina is a 4th-level spellcaster. Her spellcasting ability is Wisdom. Tissina has the following cleric spells prepared:

**Cantrips (at will):** [light](z_CLI/spells/light-xphb.md), [sacred flame](z_CLI/spells/sacred-flame-xphb.md), [thaumaturgy](z_CLI/spells/thaumaturgy-xphb.md)

**1st level (4 slots):** [command](z_CLI/spells/command-xphb.md), [inflict wounds](z_CLI/spells/inflict-wounds-xphb.md), [shield of faith](z_CLI/spells/shield-of-faith-xphb.md)

**2nd level (3 slots):** [hold person](z_CLI/spells/hold-person-xphb.md), [spiritual weapon](z_CLI/spells/spiritual-weapon-xphb.md)

***Innate Spellcasting.*** Tissina's spellcasting ability is Charisma. She can innately cast the following spells, requiring no material components:

**At will:** [thaumaturgy](z_CLI/spells/thaumaturgy-xphb.md)

***Dark Devotion.*** Tissina has advantage on saving throws against being [charmed](z_CLI/rules/conditions.md#Charmed) or [frightened](z_CLI/rules/conditions.md#Frightened).

## Actions

***Multiattack.*** Tissina makes two melee attacks.

***Dagger.*** *Melee Weapon Attack:* `+4` to hit, reach 5 ft., one creature. *Hit:* 4 (`1d4 + 2`) piercing damage. Or Ranged Weapon Attack: `+4` to hit, range 20/60 ft., one creature. *Hit:* 4 (`1d4 + 2`) piercing damage.
```
^statblock