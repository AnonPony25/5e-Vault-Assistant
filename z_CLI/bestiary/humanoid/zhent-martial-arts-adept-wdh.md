---
obsidianUIMode: preview
cssclasses:
- json5e-monster
tags:
- ttrpg-cli/compendium/src/5e/wdh
- ttrpg-cli/monster/cr/3
- ttrpg-cli/monster/size/medium
- ttrpg-cli/monster/type/humanoid/any-race
aliases:
- "Zhent Martial Arts Adept"
---
# Zhent Martial Arts Adept
*Source: Waterdeep: Dragon Heist p. 160*  

The adepts believe that Manshoon should be the Zhantarim leader and serves him fearlessly.

```ad-statblock
title: Zhent Martial Arts Adept
![](https://raw.githubusercontent.com/5etools-mirror-3/5etools-img/main/bestiary/tokens/WDH/Zhent%20Martial%20Arts%20Adept.webp#token)
*Medium humanoid (any race), Lawful Evil*

- **Armor Class** 16
- **Hit Points** 49 (`11d6 + 11`)
- **Speed** 40 ft.

|STR|DEX|CON|INT|WIS|CHA|
|:---:|:---:|:---:|:---:|:---:|:---:|
|11 (+0)|17 (+3)|13 (+1)|11 (+0)|16 (+3)|10 (+0)|

- **Proficiency Bonus** +2
- **Saving Throws** ⏤
- **Skills** [Acrobatics](z_CLI/rules/skills.md#Acrobatics) +5, [Insight](z_CLI/rules/skills.md#Insight) +5, [Stealth](z_CLI/rules/skills.md#Stealth) +5
- **Senses** passive Perception 13
- **Gear** [dart](z_CLI/items/dart-xphb.md)
- **Languages** Common, Halfling
- **Challenge** 3

## Traits

***Unarmored Defense.*** While the adept is wearing no armor and wielding no shield, its AC includes its Wisdom modifier.

***Special Equipment.*** The adept wears a [teleporter ring](z_CLI/items/teleporter-ring-wdh.md).

***Halfling Nimbleness.*** The adept

***Brave.*** The adept

## Actions

***Multiattack.*** The adept makes three unarmed strikes or three dart attacks.

***Unarmed Strike.*** *Melee Weapon Attack:* `+5` to hit, reach 5 ft., one target. *Hit:* 7 (`1d8 + 3`) bludgeoning damage. If the target is a creature, the adept can choose one of the following additional effects:

- The target must succeed on a DC 13 Strength saving throw or drop one item it is holding (adept's choice).  
- The target must succeed on a DC 13 Dexterity saving throw or be knocked [prone](z_CLI/rules/conditions.md#Prone).  
- The target must succeed on a DC 13 Constitution saving throw or be [stunned](z_CLI/rules/conditions.md#Stunned) until the end of the adept's next turn.  

***Dart.*** *Ranged Weapon Attack:* `+5` to hit, range 20/60 ft., one target. *Hit:* 5 (`1d4 + 3`) piercing damage.

## Reactions

***Deflect Missile.*** In response to being hit by a ranged weapon attack, the adept deflects the missile. The damage it takes from the attack is reduced by `1d10 + 3`. If the damage is reduced to 0, the adept catches the missile if it's small enough to hold in one hand and the adept has a hand free.
```
^statblock