---
obsidianUIMode: preview
cssclasses:
- json5e-background
tags:
- ttrpg-cli/background
- ttrpg-cli/compendium/src/5e/xphb
aliases:
- "Sage"
---
# Sage
*Source: Player's Handbook (2024) p. 183. Available in the <span title='Systems Reference Document (5.2)'>SRD</span> and the Free Rules (2024)*  

**Ability Score Increase**: Choose an ability score to increase by 2 and another ability score to increase by 1 from among Constitution, Intelligence, and Wisdom.; Choose an ability score to increase by 1, another ability score to increase by 1, and another ability score to increase by 1 from among Constitution, Intelligence, and Wisdom.

- **Ability Scores.** Constitution, Intelligence, Wisdom  
- **Feat.** [Magic Initiate](z_CLI/feats/magic-initiate-xphb.md) (Wizard)  
- **Skill Proficiencies.** [Arcana](z_CLI/rules/skills.md#Arcana), [History](z_CLI/rules/skills.md#History)  
- **Tool Proficiency.** [Calligrapher's Supplies](z_CLI/items/calligraphers-supplies-xphb.md)  
- **Equipment.** Choose A or B: (A) [Quarterstaff](z_CLI/items/quarterstaff-xphb.md), [Calligrapher's Supplies](z_CLI/items/calligraphers-supplies-xphb.md), [Book](z_CLI/items/book-xphb.md) (history), [Parchment](z_CLI/items/parchment-xphb.md) (8 sheets), [Robe](z_CLI/items/robe-xphb.md), 8 GP; or (B) 50 GP  

You spent your formative years traveling between manors and monasteries, performing various odd jobs and services in exchange for access to their libraries. You whiled away many a long evening studying books and scrolls, learning the lore of the multiverse—even the rudiments of magic—and your mind yearns for more.