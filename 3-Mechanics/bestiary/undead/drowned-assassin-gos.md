...
- **Armor Class** 14 ([leather armor](z_CLI/items/leather-armor-xphb.md))
...
- **Skills** [Intimidation](z_CLI/rules/skills.md#Intimidation) +5, [Stealth](z_CLI/rules/skills.md#Stealth) +5
...
- **Gear** [dagger](z_CLI/items/dagger-xphb.md), [hand crossbow](z_CLI/items/hand-crossbow-xphb.md)
...
***Multiattack.*** The drowned assassin makes two hand crossbow attacks or two dagger attacks. It can then take the [Dash](z_CLI/rules/actions.md#Dash), [Disengage](z_CLI/rules/actions.md#Disengage), or [Hide](z_CLI/rules/actions.md#Hide) action.

***Hand Crossbow.*** *Ranged Weapon Attack:* `+5` to hit, range 30/120 ft., one target. *Hit:* 6 (`1d6 + 3`) piercing damage plus 3 (`1d6`) poison damage, and the target must succeed on a DC 12 Constitution saving throw or contract [bluerot](z_CLI/rules/diseases.md#Bluerot) (see the "Bluerot" in notes).

***Dagger.*** *Melee Weapon Attack:* `+5` to hit, reach 5 ft., one target. *Hit:* 5 (`1d4 + 3`) piercing damage plus 9 (`2d8`) poison damage, and the target must succeed on a DC 12 Constitution saving throw or contract [bluerot](z_CLI/rules/diseases.md#Bluerot) (see the "Bluerot" in notes).

***Reveal (1/Day).*** The drowned assassin removes its mask, revealing its rotting face. Each creature of the assassin's choice within 30 feet of it that can see the assassin must succeed on a DC 13 Wisdom saving throw or be [frightened](z_CLI/rules/conditions.md#Frightened) until the end of its next turn.
```
^statblock