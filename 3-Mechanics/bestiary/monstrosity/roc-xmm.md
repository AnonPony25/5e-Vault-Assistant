...
***Talons.*** *Melee Attack Roll:* `+13`, reach 5 ft. *Hit:* 23 (`4d6 + 9`) Slashing damage. If the target is a Huge or smaller creature, it has the [Grappled](z_CLI/rules/conditions.md#Grappled) condition (escape DC 19) from both talons, and it has the [Restrained](z_CLI/rules/conditions.md#Restrained) condition until the grapple ends.

## Bonus Actions

***Swoop (Recharge 5-6).*** If the roc has a creature [Grappled](z_CLI/rules/conditions.md#Grappled), the roc flies up to half its [Fly Speed](z_CLI/rules/variant-rules/fly-speed-xphb.md) without provoking [Opportunity Attacks](z_CLI/rules/actions.md#Opportunity%20Attack) and drops that creature.
```
^statblock
