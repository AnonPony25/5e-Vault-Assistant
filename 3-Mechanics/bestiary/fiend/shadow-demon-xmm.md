...
***Demonic Restoration.*** If the demon dies outside the Abyss, its body dissolves into ichor, and it gains a new body instantly, reviving with all its [Hit Points](z_CLI/rules/variant-rules/hit-points-xphb.md) somewhere in the Abyss.

***Incorporeal Movement.*** The demon can move through other creatures and objects as if they were [Difficult Terrain](z_CLI/rules/variant-rules/difficult-terrain-xphb.md). It takes 5 (`1d10`) Force damage if it ends its turn inside an object.

***Light Sensitivity.*** While in [Bright Light](z_CLI/rules/variant-rules/bright-light-xphb.md), the demon has [Disadvantage](z_CLI/rules/variant-rules/disadvantage-xphb.md) on ability checks and attack rolls.
...
***Shadow Stealth.*** While in [Dim Light](z_CLI/rules/variant-rules/dim-light-xphb.md) or [Darkness](z_CLI/rules/variant-rules/darkness-xphb.md), the demon takes the [Hide](z_CLI/rules/actions.md#Hide) action.
```
^statblock
